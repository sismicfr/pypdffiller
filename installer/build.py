#!/usr/bin/env
"""
This file is able to create a self contained pdffiller executable that contains all it needs,
...
def _check_build_result(dist_folder: str) -> str:
    ...
    pdffiller_bin = os.path.join(dist_folder, "pdffiller")
    if platform.system() == "Windows":
        pdffiller_bin = pdffiller_bin + ".exe"
    retcode = os.system(f'"{pdffiller_bin}" --help 2>NULL')
    if retcode != 0:
        raise SystemError(f"{pdffiller_bin}: Binary not working")
  ...
  kids=[
    StringFileInfo(
      ...
      StringTable(
        ...
        StringStruct(u'FileDescription', u'Fill pdf fields given input values'),
        StringStruct(u'FileVersion', u'{version}'),
        StringStruct(u'LegalCopyright', u'Copyright 2025 SISMIC'),
        StringStruct(u'ProductName', u'pdffiller'),
...
def build_installer(source_folder: str, work_folder: str) -> str:
    ...
    main_script_path = os.path.join(source_folder, "pdffiller", "__main__.py")
    hidden = "--collect-submodules=pdffiller.cli.commands "
